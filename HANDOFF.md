...
```
fuzz_tester.py              ← single entrypoint, ~1028 lines
│
...
report_template.html         external HTML template (loaded by generate_html, not an f-string)
```
...
6. `generate_html()` reads `report_template.html`, injects data via string `.replace()`
...
- The HTML report is generated by loading `report_template.html` and using string `.replace()` for `__VARIABLE__` placeholders. JS uses `{{}}` because it was previously an f-string. Don't introduce unescaped `{}` in the JS.
